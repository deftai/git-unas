...
A lightweight web admin UI for Ubiquiti UNAS devices. Provides a browser interface served directly on the device to perform **git clone**, **tar archiving**, and **file encryption** operations on NAS storage — plus a **scheduled backup** engine with configurable cron schedule and rolling retention.
...
Persistence
  /etc/git-unas/schedule.json   ← schedule config (survives upgrades)
  /etc/default/git-unas         ← PORT, SCHEDULE_CONFIG_PATH (conffile)
...
```sh
PORT=7892                                       # listen port
SCHEDULE_CONFIG_PATH=/etc/git-unas/schedule.json
```

### Schedule config (`/etc/git-unas/schedule.json`)
...
Changes take effect immediately via the admin UI **Save** button (no service restart required).
...
scripts/build-deb.sh --arch arm64 --version 1.0.0
# → dist/git-unas_1.0.0_arm64.deb
```
...
Tested on Ubiquiti Dream Machine / Dream Router running UniFi OS 4.x (Debian arm64). The binary is a self-contained Node.js 18 executable — no Node.js installation required on the device.
